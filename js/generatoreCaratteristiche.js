export function initGeneratoreCaratteristiche() {
  ...
  function getAsiDisponibili() {
    ...
    return soglie.filter(lv => lv <= livello).length * 2; // +2 punti per ogni ASI
  }
  ...
  function aggiornaOutput() {
    ...
    const asiDisponibili = getAsiDisponibili();
    const asiUsati = Object.values(asiAggiunti).reduce((a, b) => a + b, 0);
    const asiRimanenti = asiDisponibili - asiUsati;
    ...
    caratteristiche.forEach(stat => {
      ...
      li.innerHTML = `
        ...
        <button data-stat="${stat}" data-action="point+">+</button> 
        &nbsp; | ASI: 
        <button data-stat="${stat}" data-action="asi+">+</button>
    ...
    info.innerHTML = `
      ...
      📈 <strong>ASI Usati</strong>: ${asiUsati} / ${asiDisponibili} 
    `;
    ...
    document.querySelectorAll('button[data-stat]').forEach(btn => {
      btn.onclick = () => {
        ...
        if (action === 'point+') {
          ...
          if (asiRimanenti > 0 && valori[stat] + asiAggiunti[stat] < 20) {
            asiAggiunti[stat]++;
          }
        }
