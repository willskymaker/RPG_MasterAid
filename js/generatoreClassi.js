export function initGeneratoreClassi() {
  ...
  const selezionaClasse = (nomeClasse) => {
    const datiClasse = classi[nomeClasse];
    schedaPersonaggio.classe = nomeClasse;
  ...
  select.addEventListener('change', () => {
    selezionaClasse(select.value);
  });

  btnRandom.addEventListener('click', () => {
    const tutteLeClassi = Object.keys(classi);
    const sceltaCasuale = tutteLeClassi[Math.floor(Math.random() * tutteLeClassi.length)];
    select.value = sceltaCasuale;
    selezionaClasse(sceltaCasuale);
  });
